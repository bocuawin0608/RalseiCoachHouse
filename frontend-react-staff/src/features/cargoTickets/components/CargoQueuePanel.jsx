 ...
 */
export default function CargoQueuePanel({ status, editable = false, confirmable = false }) {
    const { tickets, loading, error, pageInfo, setPageInfo, refetch } = useCargoTickets(status);
        ...
        <Table responsive hover className="cargo-queue-table align-middle mb-0">
            ...
            <tbody>
                ...
                {tickets.map(ticket => <tr key={ticket.cargoTicketId}>
                    <td><strong>{ticket.ticketCode}</strong><div><Badge bg={status === 'ARRIVED' ? 'warning' : 'primary'}>{status === 'ARRIVED' ? 'Đã đến nơi' : 'Đang chờ'}</Badge></div></td>
                    ...
                    <td><strong>{ticket.licensePlate || 'Chưa gán xe'}</strong><small>Chuyến #{ticket.tripId || '—'}</small></td>
